...
class ToolTipLabel(val label: String, toolTipText: String) : VisTable() {
    private var fieldLabel = VisLabel(label)
    var fieldInfo = VisLabel(SymbolIcon.HELP.symbol)
    ...
    init {
        ...
        fieldInfo.style = VisUI.getSkin().get("tooltip", LabelStyle::class.java)
        fieldInfo.setFontScale(0.7f)
        fieldInfo.addListener(object : InputListener() {
            override fun enter(event: InputEvent?, x: Float, y: Float, pointer: Int, fromActor: Actor?) {
                super.enter(event, x, y, pointer, fromActor)
                fieldInfo.style = VisUI.getSkin().get("tooltipActive", LabelStyle::class.java)
            }
