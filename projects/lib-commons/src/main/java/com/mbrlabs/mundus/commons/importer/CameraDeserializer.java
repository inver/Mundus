...
public class CameraDeserializer extends StdDeserializer<PerspectiveCamera> {
    ...
    @Override
    public PerspectiveCamera deserialize(JsonParser p, DeserializationContext ctxt) throws IOException {
        var res = new PerspectiveCamera();

        var node = p.getCodec().readTree(p);
        res.direction.set(
                p.getCodec().readValue(node.get("viewPointPosition").traverse(), Vector3Dto.class).toVector()
        );
