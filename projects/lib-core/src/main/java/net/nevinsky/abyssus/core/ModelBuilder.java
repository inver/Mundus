 ...
 * building use the {@link #end()} method. The end method returns the model just build. Building cannot be nested, only
 * one model (per ModelBuilder) can be build at the time. The same ModelBuilder can be used to build multiple models
 * sequential. Use the {@link #node()} method to start a new node. Use one of the #part(...) methods to add a part
 * within a node. The {@link #part(String, int, VertexAttributes, Material)} method will return a
...
public class ModelBuilder {
    ...
    public Model createSphere(float width, float height, float depth, int divisionsU, int divisionsV, int primitiveType,
        ...
        begin();
        part("sphere", primitiveType, attributes, material).sphere(width, height, depth, divisionsU, divisionsV,
                angleUFrom,
                angleUTo, angleVFrom, angleVTo);
        return end();
    }
