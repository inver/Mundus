...
import com.mbrlabs.mundus.commons.loader.obj.builder.ObjObject;
...
public class ObjParser {
    ...
    public ObjObject parse() throws IOException {
        var res = new ObjObject();
        try (var br = new BufferedReader(new FileReader(objFile))) {
            parse(br, res);
        }
        return res;
    }

    private void parse(BufferedReader br, BuilderInterface builder) throws IOException {
        int lineCount = 0;
        String line;
        while (true) {
            line = br.readLine();
            ...
            // group name, we seem to get a lot of times when we have
    ...
    // >     surface.
    private void processVertex(String line, BuilderInterface builder) {
    ...
    private void processVertexTexture(String line, BuilderInterface builder) {
    ...
    // 
    //     w is a value for the depth of the texture. The default is 0.
    private void processVertexNormal(String line, BuilderInterface builder) {
    ...
    private void processFace(String line, BuilderInterface builder) {
        line = line.substring(OBJ_FACE.length()).trim();
        int[] verticeIndexAry = ObjStringUtils.parseListVerticeNTuples(line, 3);
//        String parsedList = "";
//        int loopi = 0;
...
//        System.err.println("Parsed as "+parsedList);
    ...
    //     f 2 6 7 3
    //     # 6 elements
    private void processGroupName(String line, BuilderInterface builder) {
        String[] groupnames = ObjStringUtils.parseWhitespaceList(line.substring(OBJ_GROUP_NAME.length()).trim());
    ...
    private void processObjectName(String line, BuilderInterface builder) {
    ...
    //     f 4    // 4 3    // 3 5    // 5 6    // 6
    //     # 2 elements
    private void processSmoothingGroup(String line, BuilderInterface builder) {
    ...
    private void processPoint(String line, BuilderInterface builder) {
        line = line.substring(OBJ_POINT.length()).trim();
        int[] values = ObjStringUtils.parseListVerticeNTuples(line, 1);
        builder.addPoints(values);
    }

    private void processLine(String line, BuilderInterface builder) {
        line = line.substring(OBJ_LINE.length()).trim();
        int[] values = ObjStringUtils.parseListVerticeNTuples(line, 2);
    ...
    private void processMaterialLib(String line, BuilderInterface builder) throws IOException {
        String[] matlibnames = ObjStringUtils.parseWhitespaceList(line.substring(OBJ_MTLLIB.length()).trim());
        if (matlibnames == null) {
            return;
        }
        ...
        for (String matlibname : matlibnames) {
            try {
                parseMtlFile(matlibname, builder);
            } catch (FileNotFoundException e) {
                log.error("Can't find material file name='" + matlibname + "', e=" + e);
            }
        }
    }

    private void processUseMaterial(String line, BuilderInterface builder) {
        builder.setCurrentUseMaterial(line.substring(OBJ_USEMTL.length()).trim());
    }

    private void processMapLib(String line, BuilderInterface builder) {
        String[] maplibnames = ObjStringUtils.parseWhitespaceList(line.substring(OBJ_MAPLIB.length()).trim());
        builder.addMapLib(maplibnames);
    }

    private void processUseMap(String line, BuilderInterface builder) {
    ...
    // ----------------------------------------------------------------------
    private void parseMtlFile(String mtlFilename, BuilderInterface builder) throws IOException {
        int lineCount = 0;
        ...
        while (true) {
            ...
            if (line.startsWith("#")) // comment
            {
                ...
                processNewmtl(line, builder);
            } else if (line.startsWith(MTL_KA)) {
                processReflectivityTransmissivity(MTL_KA, line, builder);
            } else if (line.startsWith(MTL_KD)) {
                processReflectivityTransmissivity(MTL_KD, line, builder);
            } else if (line.startsWith(MTL_KS)) {
                processReflectivityTransmissivity(MTL_KS, line, builder);
            } else if (line.startsWith(MTL_TF)) {
                processReflectivityTransmissivity(MTL_TF, line, builder);
            } else if (line.startsWith(MTL_ILLUM)) {
                processIllum(line, builder);
            } else if (line.startsWith(MTL_D)) {
                processD(line, builder);
            } else if (line.startsWith(MTL_NS)) {
                processNs(line, builder);
            } else if (line.startsWith(MTL_SHARPNESS)) {
                processSharpness(line, builder);
            } else if (line.startsWith(MTL_NI)) {
                processNi(line, builder);
            } else if (line.startsWith(MTL_MAP_KA)) {
                processMapDecalDispBump(MTL_MAP_KA, line, builder);
            } else if (line.startsWith(MTL_MAP_KD)) {
                processMapDecalDispBump(MTL_MAP_KD, line, builder);
            } else if (line.startsWith(MTL_MAP_KS)) {
                processMapDecalDispBump(MTL_MAP_KS, line, builder);
            } else if (line.startsWith(MTL_MAP_NS)) {
                processMapDecalDispBump(MTL_MAP_NS, line, builder);
            } else if (line.startsWith(MTL_MAP_D)) {
                processMapDecalDispBump(MTL_MAP_D, line, builder);
            } else if (line.startsWith(MTL_DISP)) {
                processMapDecalDispBump(MTL_DISP, line, builder);
            } else if (line.startsWith(MTL_DECAL)) {
                processMapDecalDispBump(MTL_DECAL, line, builder);
            } else if (line.startsWith(MTL_BUMP)) {
                processMapDecalDispBump(MTL_BUMP, line, builder);
            } else if (line.startsWith(MTL_REFL)) {
                processRefl(line, builder);
            } else {
                log.warn("line " + lineCount + " unknown line |" + line + "|");
        ...
        log.info("Parse.parseMtlFile: Loaded " + lineCount + " lines");
    }

    private void processNewmtl(String line, BuilderInterface builder) {
        line = line.substring(MTL_NEWMTL.length()).trim();
        builder.newMtl(line);
    }

    private void processReflectivityTransmissivity(String fieldName, String line, BuilderInterface builder) {
        ...
        String[] tokens = ObjStringUtils.parseWhitespaceList(line.substring(fieldName.length()));
        if (null == tokens) {
        ...
        if (tokens[0].equals("spectral")) {
            ...
            log.warn("Sorry Charlie, this parse doesn't handle \'spectral\' parsing.  (Mostly because I can't find any info on the spectra.rfl file.)");
            return;
// 	    if(tokens.length < 2) {
// 		log.error( "Got spectral line with not enough tokens, need at least one token for spectral file and one value for factor, found "+(tokens.length-1)+" line = |"+line+"|");
// 		return;
// 	    }
    ...
    private void processIllum(String line, BuilderInterface builder) {
    ...
    private void processD(String line, BuilderInterface builder) {
    ...
    private void processNs(String line, BuilderInterface builder) {
        line = line.substring(MTL_NS.length()).trim();
        float exponent = Float.parseFloat(line);
        builder.setNs(exponent);
    }

    private void processSharpness(String line, BuilderInterface builder) {
        line = line.substring(MTL_SHARPNESS.length()).trim();
        float value = Float.parseFloat(line);
        builder.setSharpness(value);
    }

    private void processNi(String line, BuilderInterface builder) {
    ...
    private void processMapDecalDispBump(String fieldname, String line, BuilderInterface builder) {
    ...
    private void processRefl(String line, BuilderInterface builder) {
