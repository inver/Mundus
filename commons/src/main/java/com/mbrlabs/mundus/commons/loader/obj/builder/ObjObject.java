...
import com.mbrlabs.mundus.commons.loader.obj.BuilderInterface;
...
@Slf4j
public class ObjObject implements BuilderInterface {
    ...
    public List<VertexDto> verticesG = new ArrayList<>();
    public List<VertexTextureDto> verticesT = new ArrayList<>();
    public List<Vector3Dto> verticesN = new ArrayList<>();
    ...
    public Map<Integer, List<Face>> smoothingGroups = new HashMap<>();
    private int currentSmoothingGroupNumber = NO_SMOOTHING_GROUP;
    private List<Face> currentSmoothingGroup = null;
    public Map<String, List<Face>> groups = new HashMap<>();
    private List<String> currentGroups = new ArrayList<>();
    private List<List<Face>> currentGroupFaceLists = new ArrayList<>();
    public String objectName = null;
    private ObjMaterial currentMaterial = null;
    private ObjMaterial currentMap = null;
    public Map<String, ObjMaterial> materialLib = new HashMap<>();
    private ObjMaterial currentMaterialBeingParsed = null;
    public Map<String, ObjMaterial> mapLib = new HashMap<>();
    private ObjMaterial currentMapBeingParsed = null;
    ...
    public void addVertexGeometric(float x, float y, float z) {
        verticesG.add(new VertexDto(x, y, z));
//        log.info("Added geometric vertex " + verticesG.size() + " = " + verticesG.get(verticesG.size() - 1));
    }

    public void addVertexTexture(float u, float v) {
        verticesT.add(new VertexTextureDto(u, v));
//        log.info("Added texture  vertex " + verticesT.size() + " = " + verticesT.get(verticesT.size() - 1));
    }

    public void addVertexNormal(float x, float y, float z) {
        verticesN.add(new Vector3Dto(x, y, z));
    }

    public void addPoints(int[] values) {
        log.info("@TODO: Got " + values.length + " points in builder, ignoring");
    }

    public void addLine(int[] values) {
        log.info("@TODO: Got a line of " + values.length + " segments in builder, ignoring");
    }
...
//            log.info("Adding vertex g=" + vertexIndices[loopi] + " t=" + vertexIndices[loopi + 1] + " n=" + vertexIndices[loopi + 2]);
            int vertexIndex;
            ...
            if (((vertexIndex - 1) >= 0) && ((vertexIndex - 1) < verticesG.size())) {
                ...
                fv.vertex = verticesG.get(vertexIndex - 1);
            } else {
                log.error("Index for geometric vertex=" + vertexIndex + " is out of the current range of geometric vertex values 1 to " + verticesG.size() + ", ignoring");
            }
            ...
            if (vertexIndex != EMPTY_VERTEX_VALUE) {
                ...
                if (((vertexIndex - 1) >= 0) && ((vertexIndex - 1) < verticesT.size())) {
                    // Note: vertex indices are 1-indexed, i.e. they start at
                    // one, so we offset by -1 for the 0-indexed array lists.
                    fv.texture = verticesT.get(vertexIndex - 1);
                } else {
                    log.error("Index for texture vertex=" + vertexIndex + " is out of the current range of texture vertex values 1 to " + verticesT.size() + ", ignoring");
                }
            }
            ...
            if (vertexIndex != EMPTY_VERTEX_VALUE) {
                ...
                if (((vertexIndex - 1) >= 0) && ((vertexIndex - 1) < verticesN.size())) {
                    // Note: vertex indices are 1-indexed, i.e. they start at
                    // one, so we offset by -1 for the 0-indexed array lists.
                    fv.normal = verticesN.get(vertexIndex - 1);
                } else {
                    log.error("Index for vertex normal=" + vertexIndex + " is out of the current range of vertex normal values 1 to " + verticesN.size() + ", ignoring");
                }
            }

            if (fv.vertex == null) {
                log.error("Can't add vertex to face with missing vertex!  Throwing away face.");
                faceErrorCount++;
                return;
            }
...
//        log.info("Parsed face=" + face);
    ...
    public void addMapLib(String[] names) {
        if (null == names) {
            log.info("@TODO: ERROR! Got a maplib line with null names array - blank group line? (i.e. \"g\\n\" ?)");
            return;
        }
        if (names.length == 1) {
            log.info("@TODO: Got a maplib line with one name=|" + names[0] + "|");
            return;
        }
        log.info("@TODO: Got a maplib line;");
        for (int loopi = 0; loopi < names.length; loopi++) {
            log.info("        names[" + loopi + "] = |" + names[loopi] + "|");
        }
    }
...
//        if (null == names) {
//            log.info("@TODO: Got a mtllib line with null names array - blank group line? (i.e. \"g\\n\" ?)");
//            return;
//        }
//        if (names.length == 1) {
//            log.info("@TODO: Got a mtllib line with one name=|" + names[0] + "|");
//            return;
//        }
//        log.info("@TODO: Got a mtllib line;");
//        for (int loopi = 0; loopi < names.length; loopi++) {
//            log.info("        names[" + loopi + "] = |" + names[loopi] + "|");
//        }
//    }
    public void newMtl(String name) {
        currentMaterialBeingParsed = new ObjMaterial(name);
    ...
    public void doneParsingObj(String filename) {
        log.info("Loaded filename '" + filename + "' with " + verticesG.size() + " verticesG, " + verticesT.size() + " verticesT, " + verticesN.size() + " verticesN and " + faces.size() + " faces, of which " + faceTriCount + " triangles, " + faceQuadCount + " quads, and " + facePolyCount + " with more than 4 points, and faces with errors " + faceErrorCount);
    }
}