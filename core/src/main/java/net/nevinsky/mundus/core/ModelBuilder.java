...
import net.nevinsky.mundus.core.builder.*;
import net.nevinsky.mundus.core.mesh.Mesh;
import net.nevinsky.mundus.core.mesh.MeshBuilder;
...
public class ModelBuilder {
    ...
    /**
     * Adds the nodes of the specified model to a new node of the model being build. After this method the given model
     * can no longer be used. Do not call the {@link Model#dispose()} method on that model.
     *
    ...
    /**
     * Adds the specified mesh part to the current node. The Mesh will be managed by the model and disposed when the
     * model is disposed. The resources the Material might contain are not managed, use {@link #manage(Disposable)} to
     * add those to the model.
     *
    ...
    /**
     * Convenience method to create a model with a single node containing a box shape. The resources the Material might
    ...
    public Model createBox(float width, float height, float depth, int primitiveType, final Material material,
        ...
        begin();
        MeshPartBuilder partBuilder = part("box", primitiveType, attributes, material);
        BoxShapeBuilder.build(partBuilder, width, height, depth);
        return end();
    }

    /**
     * Convenience method to create a model with a single node containing a rectangle shape. The resources the Material
     * might contain are not managed, use {@link Model#manageDisposable(Disposable)} to add those to the model.
     *
     * @param attributes bitwise mask of the {@link com.badlogic.gdx.graphics.VertexAttributes.Usage}, only Position,
     *                   Color, Normal and TextureCoordinates is supported.
     */
    ...
    /**
     * Convenience method to create a model with a single node containing a cylinder shape. The resources the Material
     * might contain are not managed, use {@link Model#manageDisposable(Disposable)} to add those to the model.
     *
     * @param attributes bitwise mask of the {@link com.badlogic.gdx.graphics.VertexAttributes.Usage}, only Position,
     *                   Color, Normal and TextureCoordinates is supported.
    ...
    public Model createCylinder(float width, float height, float depth, int divisions, int primitiveType,
                                ...
                                final long attributes, float angleFrom, float angleTo) {
        begin();
        MeshPartBuilder partBuilder = part("cylinder", primitiveType, attributes, material);
        CylinderShapeBuilder.build(partBuilder, width, height, depth, divisions, angleFrom, angleTo);
        return end();
    }

    /**
     * Convenience method to create a model with a single node containing a cone shape. The resources the Material might
     * contain are not managed, use {@link Model#manageDisposable(Disposable)} to add those to the model.
     *
     * @param attributes bitwise mask of the {@link com.badlogic.gdx.graphics.VertexAttributes.Usage}, only Position,
     *                   Color, Normal and TextureCoordinates is supported.
    ...
    public Model createCone(float width, float height, float depth, int divisions, int primitiveType,
                            ...
                            final long attributes, float angleFrom, float angleTo) {
        begin();
        MeshPartBuilder partBuilder = part("cone", primitiveType, attributes, material);
        ConeShapeBuilder.build(partBuilder, width, height, depth, divisions, angleFrom, angleTo);
        return end();
    }

    /**
     * Convenience method to create a model with a single node containing a sphere shape. The resources the Material
     * might contain are not managed, use {@link Model#manageDisposable(Disposable)} to add those to the model.
     *
     * @param attributes bitwise mask of the {@link com.badlogic.gdx.graphics.VertexAttributes.Usage}, only Position,
     *                   Color, Normal and TextureCoordinates is supported.
    ...
    public Model createSphere(float width, float height, float depth, int divisionsU, int divisionsV,
                              final Material material,
                              final long attributes, float angleUFrom, float angleUTo, float angleVFrom,
                              float angleVTo) {
        return createSphere(width, height, depth, divisionsU, divisionsV, GL20.GL_TRIANGLES, material, attributes,
                angleUFrom,
     ...
     */
    public Model createSphere(float width, float height, float depth, int divisionsU, int divisionsV, int primitiveType,
                              final Material material, final long attributes, float angleUFrom, float angleUTo,
                              float angleVFrom, float angleVTo) {
        begin();
        MeshPartBuilder partBuilder = part("sphere", primitiveType, attributes, material);
        SphereShapeBuilder.build(partBuilder, width, height, depth, divisionsU, divisionsV, angleUFrom,
                angleUTo, angleVFrom, angleVTo);
        return end();
    }

    /**
     * Convenience method to create a model with a single node containing a capsule shape. The resources the Material
     * might contain are not managed, use {@link Model#manageDisposable(Disposable)} to add those to the model.
     *
     * @param attributes bitwise mask of the {@link com.badlogic.gdx.graphics.VertexAttributes.Usage}, only Position,
     *                   Color, Normal and TextureCoordinates is supported.
    ...
    public Model createCapsule(float radius, float height, int divisions, int primitiveType, final Material material,
                               final long attributes) {
        begin();
        MeshPartBuilder partBuilder = part("capsule", primitiveType, attributes, material);
        CapsuleShapeBuilder.build(partBuilder, radius, height, divisions);
        return end();
    }

    /**
     * Resets the references to {@link Material}s, {@link Mesh}es and {@link MeshPart}s within the model to the ones
    ...
    /**
     * Convenience method to create a model with three orthonormal vectors shapes. The resources the Material might
     * contain are not managed, use {@link Model#manageDisposable(Disposable)} to add those to the model.
     *
    ...
    public Model createArrow(float x1, float y1, float z1, float x2, float y2, float z2, float capLength,
        ...
        begin();
        MeshPartBuilder partBuilder = part("arrow", primitiveType, attributes, material);
        ArrowShapeBuilder.build(partBuilder, x1, y1, z1, x2, y2, z2, capLength, stemThickness, divisions);
        return end();
    }
    ...
    /**
     * Convenience method to create a model which represents a grid of lines on the XZ plane. The resources the Material
     * might contain are not managed, use {@link Model#manageDisposable(Disposable)} to add those to the model.
     *
    ...
    public Model createLineGrid(int xDivisions, int zDivisions, float xSize, float zSize, Material material,
        ...
        float xlength = xDivisions * xSize, zlength = zDivisions * zSize, hxlength = xlength / 2, hzlength =
                zlength / 2;
